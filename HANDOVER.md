...
- 写作：`writing_state_`, `writing_answers_`
- 口语：无持久化（录音文件在浏览器内存中）

### i18n 机制
- i18n.js 定义 `en` 和 `zh` 两个词库
- `t(key)` 函数根据 `i18n.currentLang` 返回对应语言文本
- 切换语言：`switchLang('en'|'zh')` 重新渲染所有 `data-i18n` 属性

### 计时器
- timer.js 提供通用 Timer 对象，用于阅读60分钟倒计时
- 听力有自己的计时逻辑（预读30秒 + 填写10分钟）
- 写作有自己的60分钟计时
...
| writing-review.js | `renderWritingReview(testData, container)` | 写作回顾 |
| history.js | `renderHistoryPage(container)` | 历史统计 |
| app.js | `App.init()` | 应用启动入口 |
...
3. **看具体 JS 文件** — 入口函数如上表，从 app.js 开始追踪路由
4. **建议先改一个简单的** — 比如修改 i18n 词条或样式，熟悉流程
5. **memory 系统** — 本文件对应的 memory 文件在 `.claude/projects/*/memory/` 下，记录了开发过程中的所有决策和 Bug 修复
